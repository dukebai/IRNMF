function [AUC,AUPR,Acc,Sen,Spe,Pre]=ROCcompute1(predictList,trueList,plotOption)
        ...
        low=min(predictList);
         ...
         if plotOption==1
            ...
            xlabel('FPR');
            ylabel('TPR');
           legend('IRNMFVDA(AUC=0.8127)','NMFVDA(AUC=0.8008)','IMCVDA(AUC=0.7221)','CMFVDA(AUC=0.6470)','RLSMDAVDA(AUC=0.7384)')
           figure;
            plot(Sen,Pre,'--r','LineWidth',2);
            axis([0 1.00 0 1.00]);
            xlabel('Sen');
            ylabel('Pre');
%            legend('DCNMDA(AUPR=0.4442)','CNMDA(AUPR=0.4309)','RLSMDA(AUPR=0.3718)','NCPMDA(AUPR=0.2315)','IMCMDA(AUPR=0.2514)')
         end
